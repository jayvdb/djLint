...
def test_H032(runner: CliRunner, tmp_file: TextIO) -> None:
    write_to_file(tmp_file.name, b"<!DOCTYPE html>")
    result = runner.invoke(djlint, [tmp_file.name])
    assert result.exit_code == 0

    write_to_file(tmp_file.name, b"<!DOCTYPE other>")
    result = runner.invoke(djlint, [tmp_file.name])
    assert result.exit_code == 1
    assert "H032 1:" in result.output

    write_to_file(tmp_file.name, b"<!DOCTYPE html PUBLIC>")
    result = runner.invoke(djlint, [tmp_file.name])
    assert result.exit_code == 1
    assert "H032 1:" in result.output
